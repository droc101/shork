...
void consoleDraw(const ivec2 viewport)
{
    ...
    sprintf(buf, "\033[H"); // Move cursor to home position
    buf += strlen(buf);
    for (int hy = viewport[1] - 1; hy >= 0; hy--) {
        for (int x = 0; x < viewport[0]; x++) {
            ...
            sprintf(buf, "\033[48;2;%d;%d;%dm\033[38;2;%d;%d;%dm▄", pixel[0], pixel[1], pixel[2], pixelBelow[0], pixelBelow[1], pixelBelow[2]);
            buf += strlen(buf);
        }
