...
def ParseOBJ(file_path):
	...
	for idx in obj.inds:
		idx_bin_data += IntToBytes(idx)
 
	# Make the header and add the data
	bin_data = bytearray()
	bin_data += struct.pack('4s', b'MSH\0')
	bin_data.extend(IntToBytes(len(obj.inds)))
	bin_data.extend(IntToBytes(len(obj.verts)))
...
def ConvertOBJ(path):
	...
	data += IntToBytes(len(data))

	return data

if (len(sys.argv) != 3):
	print("Usage: python convert_model.py <input_obj> <output_bin>")
	sys.exit(1)

input_path = sys.argv[1]
output_path = sys.argv[2]

obj = ConvertOBJ(input_path)
